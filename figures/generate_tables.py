...
This script is a methodological skeleton.  The depolarizing column of
Table I is reproduced exactly by these formulas.  The dephasing and
amplitude-damping rows of Table II in the paper were generated by full
Qiskit / Cirq simulation with the noise models described in
App.~\ref{app:sim-params}; reviewers wishing to reproduce those exact
values should run the companion simulator notebook (see Code
Availability).  The numbers printed below provide a self-consistent
analytical sanity check that the Delta F advantage of U_RA(pi/8) is
positive and of comparable magnitude across all three noise models.
...
def avg_fidelity_dephasing(gamma_eff: float) -> float:
    """Average gate fidelity under independent per-qubit phase damping.

    Phase damping at rate gamma on each qubit; for a 2-qubit gate the
    process fidelity to the ideal is (1 - gamma)^2 (each qubit's Z error
    is uncorrelated). F_avg follows from the d=4 conversion.
    """
    F_proc = (1 - gamma_eff) ** 2
    return (4 * F_proc + 1) / 5
...
def avg_fidelity_amp_damp(p_eff: float) -> float:
    ...
    Approximation: F_proc ~ (1 - p)^2 for small p (leading order in p).
    For paper-grade values, regenerate from the Kraus simulator.
    """
    F_proc = (1 - p_eff) ** 2
    return (4 * F_proc + 1) / 5
