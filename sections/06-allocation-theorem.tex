...
%   New v1.0 contribution: the allocation theorem, scheduling algorithm,
%   and multi-layer fidelity benchmark.
%   DRAFT — to be integrated into pub1 as §6 (renumbering §5 → §5 "per-gate
%   validation"; current §6 Discussion → §7).
...
We define the per-layer cost of a tunable exchange gate
...
(Sec.~\ref{subsec:entcap}; we restrict to the entangling half
...
We test Theorem~\ref{thm:alloc} against two naive scheduling
...
$\rho_0 = \ket{+0}\!\bra{+0}$.
For each entanglement target $E_{\rm target} \in [0.25, 3.5]$, we
...
\end{itemize}
For each strategy we compute the final reduced-state fidelity
