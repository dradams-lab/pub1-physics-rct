...
The \RCT~packages the dynamics of pointer-basis coherence as a
...
realized on a range of superconducting and spin-qubit
...
The family $\URA(\theta)$ is therefore not a new gate but the
isotropic-exchange or ``$\sqrt{\textsc{swap}}$ family'' parameterized
by exchange angle; the contribution of this section is to position the
family in the coherence-budget framework of Sec.~\ref{sec:rct} and to
benchmark its fidelity advantage at $\theta < \pi/4$ under
gate-time-proportional noise in Sec.~\ref{sec:sim}.
...
Proposition~\ref{prop:conservation}: the coherence redistributed from
$\rho_S$ into $S$--$E$ correlations during a partial-exchange operation
is a strictly smaller fraction of the local $\bar\alpha$ budget than for
a full maximally-entangling operation.
...
are analogous tunable primitives, and on Sycamore-class hardware
